...
pub fn convert<T: AsRef<Path> + ?Sized>(file_path: &T, output_path: &T) -> Result<(), PngdefryError> {
    ...
    let file_path = std::ffi::CString::new(file_path_str.to_string()).map_err(|e| PngdefryError::Msg(e.to_string()))?;
    let output_path = std::ffi::CString::new(output_path_str.to_string()).map_err(|e| PngdefryError::Msg(e.to_string()))?;
    unsafe {
