...
unsigned int CRC_256[] = {
        ...
        0xe0d5e91e, 0x97d2d988, 0x9b64c2b, 0x7eb17cbd, 0xe7b82d07, 0x90bf1d91,
        ...
        0x76dc4190, 0x1db7106, 0x98d220bc, 0xefd5102a, 0x71b18589, 0x6b6b51f, 0x9fbfe4a5, 0xe8b8d433, 0x7807c9a2,
        ...
        0x94643b84, 0xd6d6a3e, 0x7a6a5aa8, 0xe40ecf0b, 0x9309ff9d, 0xa00ae27, 0x7d079eb1,
        ...
        0x9b64c2b0, 0xec63f226, 0x756aa39c, 0x26d930a, 0x9c0906a9, 0xeb0e363f, 0x72076785, 0x5005713, 0x95bf4a82,
        0xe2b87a14, 0x7bb12bae, 0xcb61b38, 0x92d28e9b, 0xe5d5be0d, 0x7cdcefb7, 0xbdbdf21,
...
int read_long(void *src) {
    return (((unsigned char *) src)[0] << 24) + (((unsigned char *) src)[1] << 16) + (((unsigned char *) src)[2] << 8) +
           ((unsigned char *) src)[3];
}
...
//        printf("%s : not found or could not be opened\n", filename);
        snprintf(error, 1024, "%s : not found or could not be opened", filename);
        return 0;
    }
...
//        printf("%s : not a PNG file\n", filename);
        snprintf(error, 1024, "%s : not a PNG file", filename);
        fclose(f);
        return 0;
    }
    ...
    if (result < 0) {
        ...
        switch (result) {
            case -1:
//                printf("%s : invalid chunk size\n", filename);
                snprintf(error, 1024, "%s : invalid chunk size", filename);
                break;
            case -2:
//                printf("%s : out of memory\n", filename);
                snprintf(error, 1024, "%s : out of memory", filename);
                break;
            case -3:
...
//                    printf("invalid chunk size\n");
                    snprintf(error, 1024, "invalid chunk size");
                    break;
                case -2:
//                    printf("out of memory\n");
                    snprintf(error, 1024, "out of memory");
                    break;
                case -3:
//                    printf("premature end of file\n");
                    snprintf(error, 1024, "premature end of file");
                    break;
                default:
//                    printf("error code %d\n", result);
...
// 			printf ("%s : ", filename);
    ...
    if (flag_List_Chunks) {
        for (i = 0; i < num_chunks; i++) {
            ...
            printf("    chunk : %c%c%c%c  length %6u  CRC32 %08X", (pngChunks[i].id >> 24) & 0xff,
                   (pngChunks[i].id >> 16) & 0xff, (pngChunks[i].id >> 8) & 0xff, pngChunks[i].id & 0xff,
                   pngChunks[i].length, pngChunks[i].crc32);
...
// 				printf ("%s : ", filename);
        ...
        if (isPhoney)
            out_length = tinfl_decompress_mem_to_mem(data_out, bytespline * imgheight + row_filter_bytes, all_idat,
                                                     total_idat_size, 0);
        else
            out_length = tinfl_decompress_mem_to_mem(data_out, bytespline * imgheight + row_filter_bytes, all_idat,
                                                     total_idat_size, TINFL_FLAG_PARSE_ZLIB_HEADER);
...
// 				printf ("%s : ", filename);
        ...
        repack_length = tdefl_compress_mem_to_mem(data_repack + 4, repack_size - 4, data_out, out_length,
